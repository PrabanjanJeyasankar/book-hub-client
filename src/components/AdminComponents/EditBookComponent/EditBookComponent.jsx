  ...
  useEffect(() => {
    const fetchBookData = async () => {
      try {
        ...
        setPopupMessageBody("Could not fetch book data. Please try again.");
    ...
    if (Object.keys(formErrors).length === 0) {
      try {
        ...
        if (response.status === 200) {
          const bookTitle = response.data?.book?.title || "Unknown Book";
          setPopupImageSrc("successImage");
          setPopupMessageTitle("Book Updated Successfully");
          setPopupMessageBody(`"${bookTitle}" has been updated successfully!`);
          setIsModalOpen(true);
        }
      } catch (error) {
